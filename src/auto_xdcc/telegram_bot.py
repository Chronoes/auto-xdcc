...
class TelegramBot(ThreadRunner):
    ...
    def _run(self):
        ...
        self.send_message(
            "-> AXDCC bot ready! <-",
            reply_markup={"keyboard": [["show list", "show list archived"]]},
    ...
    def process_message(self, message):
        try:
            args = [arg for arg in shlex.split(message["text"].replace("'", "\\'")) if arg != ""]
        except ValueError as e:
            self.logger.error("Error %s parsing message: %s", str(e), message["text"])
            return
        if args:
            try:
    ...
    def get_updates(self, last_id=None, timeout=10, allowed_updates=["message"]):
        params = {"timeout": timeout, "allowed_updates": json.dumps(allowed_updates)}
