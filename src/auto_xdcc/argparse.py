...
def searchshow_handler(args):
    ...
    def _handle_search_items(items):
        ...
        resolutions.sort()
        args.printer.list(
            "#{}: {} - latest: {:02}, resolutions: {}".format(
                global_idx, items[0].show_name, last_episode, ", ".join(map(str, resolutions))
            )
        )
...
def packlist_timer_handler(args):
    ...
    if args.type == "refresh":
        ...
        if args.off:
            ...
            args.printer.x(
                "Refresh timer enabled for packlist {} with interval {}s.".format(packlist, packlist.refresh_interval)
            )
...
def remotecontrol_link_handler(args):
    ...
    if not args.token:
        ...
        args.printer.info(
            'To link your bot with AXDCC, send message "/start" to {0} or use this URL https://t.me/{0}'.format(
                bot_info["username"]
            )
        )
...
def create_argument_parser(printer: AbstractPrinter, prog="/axdcc"):
    ...
    packlist_subparser(
        subparsers.add_parser("packlist", printer=parser.printer, aliases=["pl"]), gconfig.get().packlist_manager
    )
