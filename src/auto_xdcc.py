...
printer = auto_xdcc.printer.Printer()
printer.add_listener(HexchatPrinter())
...
if 'telegram' in config['credentials']:
    ...
    telegram_bot = TelegramBot(telegram_conf['token'], chat_id=telegram_conf.get('chat_id'))
    printer.add_listener(TelegramBotPrinter(telegram_bot))
    telegram_bot.set_chat_id_cb(config).set_arg_parser(parser).start()
